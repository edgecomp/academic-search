...
try:
    connection = dbconnector.connect(user='admin',
                            password='edgecomps@2019', host='127.0.0.1', port='5432', database='searchenginedb')
    ...
    for row in df.itertuples(index=True, name='Pandas'):
        if getattr(row, "title") is not 'none':
            data_to_insert = (getattr(row, "conference"), str(getattr(row, "year")), getattr(row, "title"), getattr(row, "abstract"))
