...
# def threaded_work(function, corpuses, conferences):
#     q = Queue()
#     pool = ThreadPool(6)
#     pool.starmap(function, zip(corpuses, conferences, itertools.repeat(q)))
#     pool.close()
#     pool.join()
...
# def tokenize_lemmatize(corpus_out, abstracts, nlp, props, stop_words):
...
#                     corpus_out = corpus_out + word + " "
#     return corpus_out


def get_corpus(df, nlp, props): # tokenize & lemmatize to processes corpus
...
# def get_keywords_score(corpus, conference, queue):
#     corpus = [corpus]
...
#         keywords_with_score.append({'conferenceName': conference, 'keyword': keyword, 'score': score})
...
def get_individual_score(doc, tfidf_vec, feature_names, conferences, queue):
    feature_index = tfidf_vec[doc, :].nonzero()[1]
    tfidf_scores = zip(feature_index, [tfidf_vec[doc, x] for x in feature_index])
    keywords_with_score = []
    for keyword, score in [(feature_names[index], score) for (index, score) in tfidf_scores]:
...
def get_keywords_score(corpuses, conferences):
    ...
    #     top_50 = scores_pandas.head(50)
    #     print(top_50)
    #     pd.concat([score_pd, top_50])
    # return score_pd


def extract_keywords(conferences, corpuses):
    # score_data = threaded_work(get_keywords_score, corpuses, conferences)
    # columns = ['conferenceName', 'keyword', 'score']
    # score_pd = pd.DataFrame(columns=columns)
